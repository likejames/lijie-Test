...
public class VisitIntentionStatisticsJdbcSink implements SinkFunction<VisitIntentionStatisticsOutput> {
    ...
    public VisitIntentionStatisticsJdbcSink(String dbDriver, String dbUrl, String dbUser, String dbPass) {
    ...
    @Override
    public void invoke(VisitIntentionStatisticsOutput value, Context context) throws Exception {
        ...
        PreparedStatement insertStmt = conn.prepareStatement("INSERT INTO " +
                "stat_visit_intention(day_str, intention, visit_cnt, day, sys_user_id, sys_company_id) " +
                "VALUES (?,?,?,?,?,?)");
        PreparedStatement updateStmt = conn.prepareStatement("UPDATE " +
                "stat_visit_intention SET visit_cnt=? " +
                "WHERE day_str = ? AND sys_user_id = ? AND sys_company_id = ? AND intention = ?");
        // 执行更新语句
        ...
        updateStmt.setString(5, value.getIntention());
        updateStmt.execute();
        // 如果update没有查到数据，那么执行插入语句
        if(updateStmt.getUpdateCount() == 0){
            insertStmt.setString(1, value.getDayStr());
            insertStmt.setString(2, value.getIntention());
